...
public record UpdateRentalDateOutput(
        String rentId, LocalDateTime startDate, LocalDateTime endDate, RentStatus status, Instant updatedAt) {

    public static UpdateRentalDateOutput from(final Rent rent) {
        return new UpdateRentalDateOutput(
                rent.getId().getValue(), rent.getStartDate(), rent.getEndDate(), rent.getStatus(), rent.getUpdatedAt());
    }
}
