...
public record UpdateCancelRentOutput(String id, RentStatus status, String cancellationReason, Instant updatedAt) {

    public static UpdateCancelRentOutput from(final Rent rent) {
        return new UpdateCancelRentOutput(
                rent.getId().getValue(), rent.getStatus(), rent.getCancellationReason(), rent.getUpdatedAt());
    }
}
